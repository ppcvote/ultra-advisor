...
const SALES_CHEATSHEET: Record<string, {
  ...
  growth: {
    ...
    color: 'blue',
    hooks: [
      { label: '回測說明', script: '本回測圖以歷史淨值資料計算，反映過去績效，過往表現不代表未來結果，投資人仍須承擔市場波動與本金損失風險。' },
      { label: '複利公式', script: '複利模型為 A = P × (1+r)^n，年化 r 與期間 n 為主要變數。實際投資需考慮稅負、手續費、匯率與標的波動。' },
      { label: '通膨對照', script: '長期投資績效須與當期通膨率相減後比較實質報酬；定存利率與股票指數年化報酬有不同風險屬性，無法直接等同比較。' },
    ],
    objections: [
      { q: '股票波動大，本金損失風險如何評估？', a: '可參考標的之標準差與最大回撤（MDD）數據；過去 20 年台股最大單年回撤曾達 -45%（2008）。投資前應評估風險承受度與投資期間。' },
      { q: '何時進場較合適？', a: '市場時機難以預測。定期定額可分散買入成本，但不保證獲利或避免損失；單筆與定期定額之選擇須視個人現金流與目標期間。' },
    ],
    closingLines: [
      '本工具為歷史回測情境模擬，非投資建議；實際投資請評估個人風險承受度。',
      '基金投資須詳閱公開說明書與投資人須知，注意配息來源、匯率風險及費用結構。',
    ],
  },
  income: {
    title: '配息型基金',
    color: 'emerald',
    hooks: [
      { label: '配息結構', script: '配息型基金的配息來自利息收入、資本利得，或本金；境外基金須揭露「本金佔配息比率」，請詳閱基金月報書。' },
      { label: '配息率定義', script: '年化配息率 = 單次配息 × 配息頻率 / 配息基準淨值；高配息率不等於高總報酬，應觀察含息報酬與淨值走勢。' },
      { label: '與不動產比較', script: '租金收入與基金配息屬不同資產類別，租金受地段與空置率影響，基金配息受標的淨值與匯率影響，兩者風險來源不同。' },
    ],
    objections: [
      { q: '配息是否包含本金？', a: '部分高配息商品配息確實含本金返還，當標的不足以支應配息時即發生「降息」或「侵蝕本金」。請查閱基金月報書揭露之配息來源組成。' },
      { q: '淨值下跌對投資人的意義？', a: '配息與淨值變動共同構成總報酬。即使持續配息，若淨值下跌幅度大於累積配息，整體仍為虧損。' },
    ],
    closingLines: [
      '配息型基金常見風險：配息率調降、淨值下跌、匯率波動、本金返還。',
      '本工具為教育目的之情境試算，非招攬投資；實際投資請依個人風險承受度，並諮詢合格投資顧問。',
    ],
  },
};
