...
const LoginForm = ({ onSuccess }: { onSuccess: () => void }) => {
  ...
  // 🆕 Google 登入：iOS Safari PWA 用 redirect，其他用 popup
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // iOS Safari / PWA 不能用 popup（被擋），改用 redirect
      ...
      if (isIOS || isStandalone) {
        ...
        // signInWithRedirect 會跳轉，這段下面的程式碼回來才會跑
      } else {
      ...
      const errorMessages: Record<string, string> = {
        ...
        'auth/popup-blocked': '瀏覽器阻擋彈窗，請改用 Email 登入或允許彈窗',
        'auth/cancelled-popup-request': '請再試一次',
        'auth/network-request-failed': '網路連線失敗',
        'auth/account-exists-with-different-credential': '此 Email 已用其他方式註冊',
      };
      setError(errorMessages[err.code] || `Google 登入失敗：${err.code || err.message || '未知錯誤'}`);
      setLoading(false);
    }
  };

  // 確保 Firestore 有 user document（Google 第一次登入要建）
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="relative my-4">
        ...
        <div className="relative flex justify-center text-xs">
          <span className="px-3 bg-slate-950 text-slate-500">或</span>
        </div>
      </div>
      ...
      <button
        ...
        disabled={loading}
        className="w-full py-3.5 bg-white hover:bg-slate-100 text-slate-900
                 rounded-xl font-bold text-base transition-all
                 disabled:opacity-50 disabled:cursor-not-allowed flex items-center
                 justify-center gap-3 border border-slate-300">
